...
def deduplicate_and_select(news_list: List[Dict], target=50) -> List[Dict]:
    seen = set()
    unique = []
...
def translate_news(news: List[Dict]):
    for item in news:
        if is_english(item["title"]): item["title"] = translate(item["title"])
        if is_english(item["summary"]): item["summary"] = translate(item["summary"])

# ========== AI 摘要 ==========
def ai_summary(news: List[Dict]) -> str:
    if not ENABLE_AI or not LLM_API_KEY: return ""
    try:
        titles = [item["title"] for item in news[:15]]
        prompt = (
